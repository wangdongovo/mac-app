...
import { LayoutGrid, Copy, Check, Server, Network, Terminal } from 'lucide-react';
             ...
             <div className="p-4 rounded-xl">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {systemInfo?.serialNumber && (
                        <button 
                            onClick={() => handleCopy(systemInfo.serialNumber, 'serial')}
                            className="text-gray-400 hover:text-blue-500 transition-colors cursor-pointer"
                            title="复制序列号"
                        >
                            {copiedField === 'serial' ? <Check className="w-4 h-4 text-green-500"/> : <Copy className="w-4 h-4"/>}
                        </button>
                    )}
                </div>
             </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {systemInfo?.network.ip && (
                        <button 
                            ...
                            title="复制 IP"
                        >
                            {copiedField === 'ip' ? <Check className="w-4 h-4 text-green-500"/> : <Copy className="w-4 h-4"/>}
                        </button>
                    )}
                </div>
             </div>
             ...
             <div className="p-4  rounded-xl">
                ...
                <div className="text-sm font-bold text-gray-900 break-all">
                    {systemInfo?.network.dns.length > 0 ? systemInfo.network.dns.join(', ') : '未知'}
                </div>
             </div>
          </div>
        </div>
